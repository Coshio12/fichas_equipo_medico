    ...
    <x-app-layout>
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight flex items-center justify-center">
                {{__('DATOS TECNICOS PARA: ')}}{{$equipo->nombre_equipo}}
            </h2>
        </x-slot>

        <div>
            @include('equipos.datos_tecnicos.crear')
            @include('equipos.datos_tecnicos.edit')
            @include('equipos.datos_tecnicos.delete')
        </div>
                ...
                <table class="min-w-full bg-white">
                    <thead>
                        <tr class="bg-[#34bcb0] text-center text-lg md:text-lg font-thin text-white">
                            <th scope="col" class="py-5 border-r border-gray-300">CARACTERISTICA</th>
                            <th scope="col" class="py-5 border-r border-gray-300">VALOR</th>
                    ...
                    <tbody>
                        @if ($datos->isEmpty())
                            <tr class="text-center">
                                <td colspan="3" class="py-4 border-b border-gray-300 text-gray-500">
                                    No se registro ningun dato tecnico
                                </td>
                            </tr>
                        @endif
                        ...
                        @foreach ($datos as $datos)
                            <tr class="text-center">
                                <td class="py-4 border-b border-gray-300 ">
                                    {{$datos->atributo}}
                                </td>
                                <td class="py-4 border-b border-gray-300 ">
                                    {{$datos->valor}}
                                ...
                                <td class="py-4 border-b border-gray-300 ">
                                    <div class="flex space-x-2 justify-center">
                                        <button type="button" class="p-2.5 bg-amber-500 rounded-xl hover:rounded-3xl hover:bg-amber-600 transition-all duration-300 text-white w-16 h-12 flex items-center justify-center"
                                        data-id="{{$datos->id_datos}}"
                                        data-atributo="{{$datos->atributo}}"
