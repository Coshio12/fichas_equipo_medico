    ...
    <div class="modal-container w-full py-2 bg-white cursor-default pointer-events-auto relative rounded-xl mx-auto max-w-sm border-x-8 border-gray-700">
        ...
        <div class="space-y-2 p-2">
            <div class="p-2 space-y-2 text-center">
                <h2 class="text-xl font-bold tracking-tight" id="page-action.heading">Crear Categoría</h2>
            </div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <div class="grid grid-cols-1 place-items-center px-4 py-2">
                <form action="{{ route('categorias.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="nombre" class="text-gray-400 text-lg">NOMBRE DE LA CATEGORIA 
                            <span class="text-red-600 inline-block p-1 text-sm">*</span>
                        </label>
                        <input
                            id="nombre_categoria"
                            name="nombre_categoria"
                            class="border p-3 shadow-md border-gray-700 placeholder:text-base focus:outline-none ease-in-out duration-300 rounded-lg w-full mt-3"
                            type="text"
